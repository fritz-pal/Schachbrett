...
public class EvalBar extends JPanel {
  ...
  public EvalBar(Game game, Window window) {
    ...
    this.game = game;
    this.setBounds((window.getContentPane().getWidth() - window.getBoardSize()) / 2 - 50,
        (window.getContentPane().getHeight() - window.getBoardSize()) / 2, 40,
        window.getBoardSize());
    ...
    new Timer(10, event -> {
      if (Math.abs(bar.getHeight() - interpolation) <= 4) {
        bar.setBounds(0, 0, 40,
            (isRotated ? (this.getHeight() - interpolation) : interpolation));
      } else {
        bar.setBounds(0, 0, 40,
            ((isRotated ? (this.getHeight() - interpolation) : interpolation) +
                bar.getHeight() * 3) / 4);
  ...
  public void setEval(int cp, int mate) {
    ...
    if (cp < 1000 && cp > -1000) {
      whiteLabel.setText(
          new DecimalFormat("0.0", new DecimalFormatSymbols(Locale.ENGLISH)).format(cp / 100.0));
      blackLabel.setText(
          new DecimalFormat("0.0", new DecimalFormatSymbols(Locale.ENGLISH)).format(cp / 100.0));
    } else {
