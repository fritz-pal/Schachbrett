...
public class Tile extends JButton {
  ...
  private ImageIcon legalMoveIconImage() {
    BufferedImage img =
        new BufferedImage(this.getWidth(), this.getWidth(), BufferedImage.TYPE_INT_ARGB);
    Graphics2D imgGraphics = img.createGraphics();
    imgGraphics.setColor(new Color(0, 0, 0, 60));
    if (this.piece == null) {
      imgGraphics.fillOval(this.getWidth() / 4, this.getWidth() / 4, this.getWidth() / 2,
          this.getWidth() / 2);
    } else {
      ...
      imgGraphics.fillRect(0, this.getWidth() - borderSize, this.getWidth() - borderSize,
          borderSize);
    }
  ...
  private ImageIcon getImg() {
    ...
    if (this.legalMoveIcon) {
      imgGraphics.drawImage(this.legalMoveIconImage().getImage(), 0, 0, null);
    }
    if (piece != null) {
      ImageIcon pieceImg = new ImageIcon(
          ImagePath.getPieceImage(piece.type(), piece.isWhite(), game.isRotated(piece.isWhite())));
