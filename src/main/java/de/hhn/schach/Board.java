...
public class Board implements Cloneable {
  ...
  public Board(Game game, String fen) {
    ...
    blackCastleK = fenParts[2].contains("k");
      ...
      if (fenParts[3].equals("-")) {
          ...
          enCroissant = new Vec2(fenParts[3]);
      }
  }

  public Board(Game game, Map<Vec2, Piece> pieces, boolean whiteTurn, Vec2 enCroissant,
               boolean whiteCastleQ, boolean whiteCastleK, boolean blackCastleQ,
               boolean blackCastleK) {
    this.game = game;
  ...
  public static boolean isValidFen(String fen) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (!fenParts[2].matches("[KQkq-]{1,4}")) {
          return false;
      }
  ...
  private static String getSpecialNotation(Vec2 from, List<Vec2> otherPieces, String notation) {
    boolean sameFile = false;
    boolean sameRank = false;
    for (Vec2 pos : otherPieces) {
        if (pos.getName().charAt(0) == from.getName().charAt(0)) {
            sameFile = true;
        }
        if (pos.getName().charAt(1) == from.getName().charAt(1)) {
            sameRank = true;
        }
    }
    String result = String.valueOf(notation.charAt(0));
      if (sameFile) {
          result += from.getName().charAt(0);
      }
      if (sameRank) {
          result += from.getName().charAt(1);
      }
    result += notation.substring(1);
    return result;
  }
    ...
    for (int i = 0; i < 8; i++) {
      ...
      for (int k = 0; k < fenRow.length(); k++) {
        ...
        if (Character.isDigit(c)) {
          ...
          pieces.put(new Vec2(7 - i, j), new Piece(PieceType.fromNotation(Character.toUpperCase(c)),
    ...
    for (int i = 7; i >= 0; i--) {
      ...
      for (int j = 0; j < 8; j++) {
        ...
        if (piece == null) {
          ...
          fen.append(piece.isWhite() ? piece.type().getNotation() :
              Character.toLowerCase(piece.type().getNotation()));
        }
      }
      if (empty > 0) {
        fen.append(empty);
  ...
  public String toString() {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      for (int j = 0; j < 8; j++) {
        Piece piece = pieces.get(new Vec2(i, j));
        boardString.append(piece == null ? " " : piece.isWhite() ? piece.type().getNotation() :
            Character.toLowerCase(piece.type().getNotation()));
          if (j != 7) {
              boardString.append(", ");
          }
  ...
  private List<Vec2> getCastleMoves(Vec2 pos) {
    ...
    if (pos.equals(new Vec2(0, 4)) && whiteTurn && !isInCheck(true)) {
      if (whiteCastleK && !occupied(new Vec2(0, 5)) && !occupied(new Vec2(0, 6)) &&
          !ableToTakeKing(pos, new Vec2(0, 5))) {
        castleMoves.add(new Vec2(0, 6));
      }
      if (whiteCastleQ && !occupied(new Vec2(0, 3)) && !occupied(new Vec2(0, 2)) &&
          !occupied(new Vec2(0, 1)) && !ableToTakeKing(pos, new Vec2(0, 3))) {
        castleMoves.add(new Vec2(0, 2));
      }
    } else if (pos.equals(new Vec2(7, 4)) && !whiteTurn && !isInCheck(false)) {
      if (blackCastleK && !occupied(new Vec2(7, 5)) && !occupied(new Vec2(7, 6)) &&
          !ableToTakeKing(pos, new Vec2(7, 5))) {
        castleMoves.add(new Vec2(7, 6));
      }
      if (blackCastleQ && !occupied(new Vec2(7, 3)) && !occupied(new Vec2(7, 2)) &&
          !occupied(new Vec2(7, 1)) && !ableToTakeKing(pos, new Vec2(7, 3))) {
  ...
  public String getPgn() {
    ...
    if (game.isAgainstEngine() && game.isEngineWhite()) {
      ...
      pgn.append("[White \"").append(game.getName(true).isBlank() ? "?" : game.getName(true))
          .append("\"]\n");
      pgn.append("[WhiteElo \"").append(game.getElo(true) == -1 ? "?" : game.getElo(true))
          .append("\"]\n");
    }
    if (game.isAgainstEngine() && !game.isEngineWhite()) {
      ...
      pgn.append("[Black \"").append(game.getName(false).isBlank() ? "?" : game.getName(false))
          .append("\"]\n");
      pgn.append("[BlackElo \"").append(game.getElo(false) == -1 ? "?" : game.getElo(false))
          .append("\"]\n");
    }
    pgn.append("[Result \"").append(result.getNotation()).append("\"]\n");
      if (!fromFen.isEmpty()) {
          pgn.append("[FEN \"").append(fromFen).append("\"]\n");
      }
    pgn.append("\n");
    int count = 1;
  ...
  private List<Vec2> getPawnMoves(Vec2 pos) {
    ...
    if ((pos.getX() == 1 && getPiece(pos).isWhite()) ||
        (pos.getX() == 6 && !getPiece(pos).isWhite())) {
      move = new Vec2(pos.getX() + (getPiece(pos).isWhite() ? 2 : -2), pos.getY());
      if (!occupied(move) &&
          !occupied(new Vec2(pos.getX() + (getPiece(pos).isWhite() ? 1 : -1), pos.getY()))) {
        moves.add(move);
      }
    }
    if (pos.getY() - 1 >= 0) {
      ...
      if ((occupied(move) && getPiece(move).isWhite() != getPiece(pos).isWhite()) ||
          (enCroissant != null && enCroissant.equals(move))) {
        moves.add(move);
      }
    }
  ...
  private List<Vec2> canOtherPiecesGetTo(Vec2 from, Vec2 to, Piece piece) {
    ...
    for (Vec2 pos : pieces.keySet()) {
      ...
      Piece p = pieces.get(pos);
        ...
        if (getAllPseudoLegalMoves(pos).contains(to)) {
            other.add(pos);
        }
    }
    return other;
  ...
  public void move(Vec2 from, Vec2 to, boolean isMainBoard, PieceType promotion) {
    ...
    Piece piece = pieces.get(from);
      if (piece == null) {
          throw new IllegalArgumentException(
              "Illegal move: " + from.getName() + to.getName() + " (no piece at " + from.getName() +
                  ")");
      }
      if (!piece.isWhite()) {
          moveNumber++;
    ...
    String notation = "";
      if (isMainBoard) {
          notation = makeNotation(piece, from, to);
      }

    // 50 moves rule
      if (occupied(to) || piece.type().equals(PieceType.PAWN)) {
          movesWithoutCaptureOrPawnMove = 0;
      } else {
          movesWithoutCaptureOrPawnMove++;
      }
      if (movesWithoutCaptureOrPawnMove >= 100) {
          result = Result.DRAWBYFIFTYMOVESRULE;
      }
      ...
      if (piece.type().equals(PieceType.PAWN)) {
          if ((to.getX() == 0 && !piece.isWhite()) || (to.getX() == 7 && piece.isWhite())) {
              if (promotion != null) {
                  ...
                  throw new IllegalArgumentException(
                      "Illegal move: " + from.getName() + to.getName() + " (no promotion)");
              }
          }
          if (to.equals(enCroissant)) {
              pieces.remove(new Vec2(to.getX() + (piece.isWhite() ? -1 : 1), to.getY()));
          }
    ...
    if (piece.type().equals(PieceType.KING)) {
      ...
      if (from.equals(new Vec2(0, 0))) {
          whiteCastleQ = false;
      }
      if (from.equals(new Vec2(0, 7))) {
          whiteCastleK = false;
      }
      if (from.equals(new Vec2(7, 0))) {
          blackCastleQ = false;
      }
      if (from.equals(new Vec2(7, 7))) {
          blackCastleK = false;
      }
    ...
    if (isMainBoard) {
      ...
      moveHistory.add(move);
        ...
        if (isInsufficientMaterial()) {
            result = Result.DRAWBYINSUFFICIENTMATERIAL;
        }
  ...
  private boolean isInsufficientMaterial() {
    ...
    for (Piece piece : pieces.values()) {
      ...
      if (numBishopsWhite >= 2 || numBishopsBlack >= 2) {
          return false;
      }
      if (numKnightsBlack > 2 || numKnightsWhite > 2) {
          return false;
      }
      if (numBishopsWhite == 1 && numKnightsWhite == 1) {
          return false;
      }
  ...
  private void checkIfRepetition(String fen) {
    ...
    for (Move m : moveHistory) {
      ...
      if (repetition >= 3) {
          result = Result.DRAWBYTHREEFOLDREPETITION;
      }
  }
  ...
  private String makeNotation(Piece piece, Vec2 from, Vec2 to) {
    String notation = piece.type().getNotation() +
        (occupied(to) || (to.equals(enCroissant) && piece.type() == PieceType.PAWN) ? "x" : "") +
        to.getName();
    if (notation.startsWith("P")) {
      notation = notation.substring(1);
        if (notation.startsWith("x")) {
            notation = from.getName().charAt(0) + notation;
        }
    } else {
      List<Vec2> otherPieces = canOtherPiecesGetTo(from, to, piece);
      if (!otherPieces.isEmpty()) {
        if (otherPieces.size() == 1) {
          notation = String.valueOf(notation.charAt(0)) + from.getName().charAt(
              (otherPieces.get(0).getName().charAt(0) != from.getName().charAt(0)) ? 0 : 1) +
              notation.substring(1);
        } else {
          notation = getSpecialNotation(from, otherPieces, notation);
        }
      }
    }
    return notation;
  ...
  protected Board clone() throws CloneNotSupportedException {
    super.clone();
    return new Board(game, pieces, whiteTurn, enCroissant, whiteCastleQ, whiteCastleK, blackCastleQ,
        blackCastleK);
  }
